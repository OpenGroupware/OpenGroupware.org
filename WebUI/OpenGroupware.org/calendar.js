...
function setInitialDate() {
    ...
    // split("-") DIVIDES inDate INTO ARRAY (YEAR,MONTH,DAY)
    myTmpDate = inDate.split("-");

    // if (inDate.indexOf('-') && inDate.length > 5) {
    if (myTmpDate.length == 3) {

        // POSSIBLE parseInt() ERROR FOR "08" und "09"
        // -> LEADING 0 CUT AT inMonth AND inDay

        // inYear = inDate.substring(0, inDate.indexOf("-"));
        // inYear = parseInt(inYear);
        inYear = parseInt(myTmpDate[0]);

        // inMonth = inDate.substring(inDate.indexOf("-") + 1, inDate.lastIndexOf("-"));
        // inMonth = parseInt(inMonth);
        inMonth = myTmpDate[1];
        if (inMonth.charAt(0) == "0") inMonth = inMonth.substr(1,inMonth.length-1);
        inMonth = parseInt(inMonth);

        // inDay = inDate.substring(inDate.lastIndexOf("-") + 1, inDate.length);
        // inDay = parseInt(inDay);
        inDay = myTmpDate[2];
        if (inDay.charAt(0) == "0") inDay = inDay.substr(1,inDay.length-1);
        inDay = parseInt(inDay);

        myActDate = new Date();  // ERROR HANDLING
        day   = (inDay)   ? inDay       : myActDate.getDay();
        month = (inMonth) ? (inMonth-1) : (myActDate.getMonth() -1);
        year  = (inYear)  ? inYear      : myActDate.getYear();
...
function createWeekdayList() {
    ...
    // IF SPANISH
        ...
        weekdayList  = new Array('Domingo', 'Lunes', 'Martes', 'Mi‚rcoles', 'Jueves', 'Viernes', 'Sßbado')
