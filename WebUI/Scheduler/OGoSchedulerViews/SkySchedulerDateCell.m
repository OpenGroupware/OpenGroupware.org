...
- (void)_appendParticipant:(id)_person login:(id)_loginPKey 
  owner:(id)_ownerPKey showFullName:(BOOL)_showFull
  toResponse:(WOResponse *)_response inContext:(WOContext *)_ctx
{
  ...
  isDeleted = (dbStatus != nil)
    ? [dbStatus isEqualToString:@"archived"]
    : NO;
...
- (NSString *)fromDateStringForAppointment:(id)_apt
  ...
  inContext:(WOContext *)_ctx
{
  ...
  if (allDay) {
    if ([sD yearOfCommonEra] < [wD yearOfCommonEra]) 
      fm = @"%Y-%m-%d";
    else if (([sD dayOfYear]       <  [wD dayOfYear]) &&
...
- (NSString *)endDateStringForAppointment:(id)_apt
  showAMPM:(BOOL)_showAMPM
  inContext:(WOContext *)_ctx
{
  // TODO: should be a formatter?
  ...
  if (allDay) {
    if ([eD yearOfCommonEra] > [wD yearOfCommonEra]) 
      fm = @"%Y-%m-%d";
    else if ([wD dayOfYear] < [eD dayOfYear] &&
...
- (NSString *)titleStringForAppointment:(id)_apt maxLength:(int)_len
  inContext:(WOContext *)_ctx
{
  NSString    *t = nil;
  WOComponent *co;
...
- (NSString *)resourceStringForAppointment:(id)_apt maxLength:(int)_len
  inContext:(WOContext *)_ctx
{
  NSString *r = nil;
...
- (NSString *)shortTextForAppointment:(id)_apt newline:(BOOL)_nl
  showFullNames:(BOOL)_showFull
  showAMPM:(BOOL)_showAMPM
  inContext:(WOContext *)_ctx
{
...
- (void)appendDAToResponse:(WOResponse *)_response inContext:(WOContext *)_ctx
  appointment:(id)_apt
{
  NSString *url;
  int      oid;
  ...
  alt = [self shortTextForAppointment:_apt newline:YES showFullNames:showFull
              showAMPM:showAMPM inContext:_ctx];
  ...
  if ([[[_ctx session] valueForKey:@"isJavaScriptEnabled"] boolValue]) {
    ...
    alt = [self shortTextForAppointment:_apt newline:NO
                showFullNames:showFull showAMPM:showAMPM inContext:_ctx];

    if ([alt length] > 0) {
      alt = [[alt componentsSeparatedByString:@"'"]
                  componentsJoinedByString:@"&rsquo;"];
...
- (void)appendComponentActionToRespons:(WOResponse *)_response
  inContext:(WOContext *)_ctx
  appointment:(id)_apt
{
  NSString     *url;
  ...
  alt = [self shortTextForAppointment:_apt newline:YES
              showFullNames:showFull showAMPM:showAMPM inContext:_ctx];
  ...
  if ([[[_ctx session] valueForKey:@"isJavaScriptEnabled"] boolValue]) {
    ...
    alt = [self shortTextForAppointment:_apt newline:NO showFullNames:showFull
                showAMPM:showAMPM inContext:_ctx];
...
- (void)appendIconToResponse:(WOResponse *)_response inContext:(WOContext *)_ctx
  appointment:(id)_apt
{
  NSString *src;
  NSArray  *languages;
  ...
  if ([src length] < 1)
    src = @"apt_10x10.gif";
  src = [[[_ctx application] resourceManager]
                urlForResourceNamed:src
                inFramework:nil
                languages:languages
...
- (void)appendToResponse:(WOResponse *)_response inContext:(WOContext *)_ctx {
  ...
  if (link) {
    if (componentAction)
      [self appendComponentActionToRespons:_response inContext:_ctx
            appointment:a];
    else
  ...
  if (!shortInfo) {
    ...
    [_response appendContentString:
                 noAcc
                 ? @" <span class=\"skydatecell_titlePrivate\">"
                 : @" <span class=\"skydatecell_title\">"];
    ...
    if ((count = [p count]) == 0) {
      ...
      for (i = 0, accountCount = 0; i < count; i++) {
        ...
        if ([[participant valueForKey:@"isAccount"] boolValue]
            || [[participant valueForKey:@"isTeam"] boolValue]) {
  ...
  if (![self->isPrivate boolValueInComponent:co] && sevD) {
      ...
      if (ab != nil && ([ab length] > 0) && ![ab isEqualToString:@" "]) {
        [_response appendContentString:
                     @"<span class=\"skydatecell_absence\">"];
