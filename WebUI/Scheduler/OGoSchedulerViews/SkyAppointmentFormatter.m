...
@implementation SkyAppointmentFormatter

+ (SkyAppointmentFormatter *)printFormatterWithTitleLength:(int)_len
  includeLocation:(BOOL)_withLoc
  includeResources:(BOOL)_withRes
...
+ (SkyAppointmentFormatter *)printFormatterWithAppointment:(id)_apt
  isViewAccessAllowed:(BOOL)_canView
  addTrailingNewline:(BOOL)_addNL
  relationDate:(NSCalendarDate *)_date
  showFullNames:(BOOL)_showFullNames
{
  SkyAppointmentFormatter *f;
  NSString *res, *loc, *title;
  int      len;
  ...
  f = [SkyAppointmentFormatter printFormatterWithTitleLength:len
			       ...
			       addTrailingNewline:_addNL];
  [f setRelationDate:_date];
...
+ (SkyAppointmentFormatter *)formatterWithFormat:(NSString *)_format {
  return AUTORELEASE([(SkyAppointmentFormatter *)[SkyAppointmentFormatter alloc]
                                                 initWithFormat:_format]);
}

+ (SkyAppointmentFormatter *)formatter {
  return AUTORELEASE([[SkyAppointmentFormatter alloc] init]);
}
