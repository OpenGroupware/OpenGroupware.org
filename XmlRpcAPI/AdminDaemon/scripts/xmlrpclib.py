...
# $Id: xmlrpclib.py,v 1.1 2003/12/12 14:14:13 helge Exp $
#
...
import re, string, time, operator, base64
...
class Marshaller:
    ...
    def dump_string(self, value, escape=escape):
        ...
        # unicode encoding patch
        # always try to encode strings in Unicode
        try:
...
class Transport:
    ...
    def request(self, host, handler, request_body, verbose=1):
        ...
        self.send_basic_auth(h, self.login, self.password)
    ...
    def send_basic_auth(self, connection, login, password):
       if login is not None and password is not None:
          cred = base64.encodestring("%s:%s" % ( login, password ))
          cred = string.replace(cred, "\012", "")
          connection.putheader("Authorization", "Basic %s" % cred)
