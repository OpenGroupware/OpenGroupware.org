...
- (void)_associateCompaniesWithAppointments:(NSArray *)results 
								 ...
								 personGIDs:(NSDictionary *)pgids
									persons:(id)persons teams:(id)teams
{
	unsigned i, count;
	...
	for (i = 0, count = [results count]; i < count; i++)
	{
		...
		for (j = 0, k = 0; j < psCount; j++)
		{
			...
			if (ps[j] == nil)
			{
				...
				if (ps[j] && !viewAllowed && ![[ps[j] valueForKey:@"isAccount"] boolValue])
				{
					...
					ps[j] =	[NSDictionary dictionaryWithObjects:values forKeys:keys  count:n];
				}
			}
...
- (NSDictionary *)_fetchParticipantGIDsForAptGIDs:(NSArray *)_resultGids inContext:(id)_context
{
	...
	*(&batchSize) = gidCount > 200 ? 200 : gidCount;
		...
		
		{
			...
			*(&all) = [NSMutableArray arrayWithCapacity:64];
			...
			if ([self->sortOrderings count] > 0)
			{
				...
				NS_HANDLER
					printf("LSGetAppointmentsForGlobalIDs: %s\n", [[localException description] cString]);
				NS_ENDHANDLER;
			}
	...
	if (([participantKeys count] > 0) && ([results count] > 0))
	{
			...
			if ([personGids count] > 0)
			{
				...
				persons = [self _fetchPersonEOsForGIDs:personGids attributes:personAttrs inContext:_context];
			}
			else
				persons = nil;
			
			/* associate companies with appointments */
			[self _associateCompaniesWithAppointments:results withGIDs:resultGids  personGIDs:pgids persons:persons teams:teams];
		}
...
{
	LSRunCommandV(_ctx, @"appointment", @"get-participants",  @"appointments", _e, nil);
}
...
{
	LSRunCommandV(_ctx, @"appointment", @"get-access-team-info", @"appointments", _eos, nil);
}
...
- (void)_executeInContext:(id)_context
{
	...
	// here we handle the case where gids is null
	// this should not arrive but.... ;-)
	// in fact this mainly arrive when we receive a bad call from
	// upper layer
	if(self->gids != nil)
	{
		
		self->access = [[self _fetchAccessInfoForGlobalIDs:self->gids inContext:_context] retain];
		
		if (debugOn) 
			[self logWithFormat:@"ACCESS: %@", self->access];
	...
	{
		[self logWithFormat:@"**** _executeInContext run with self->gids == nil !!!!"];
		// here we return nil
		results = nil;
	}
...
{
	// here we handle receiving nil value
	// wich generate an error 
	
	if(_gids == nil)
	{
		[ self logWithFormat:@"**** setGlobalIDs : receive nil for gids !!!!"];
		return;
	}
