...
#import "common.h"
#import "LSSetArticleCategoryCommand.h"

@interface LSSetArticleCategoryCommand(privatMethods)
- (void)_checkCategoryNameInContext:(id)_context;
@end

@implementation LSSetArticleCategoryCommand
...
- (void)_checkCategoryNameInContext:(id)_context {
  ...
  NSNumber     *catId      = [self valueForKey:@"articleCategoryId"];
  ...
  while ((category = [catEnum nextObject])) {
    ...
    [self assert: (
                    (![scndName isEqualToString:catName])
                   ||
                    (  ([scndName isEqualToString:catName])
                     &&([[category valueForKey:@"articleCategoryId"]
                         isEqualToNumber: catId])
                    )
                   )
