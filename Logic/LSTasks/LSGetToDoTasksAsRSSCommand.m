...
@interface LSGetToDoTasksAsRSSCommand : LSGetAsRSSFeedCommand
{
  NSString         *accountId;
...
@implementation LSGetToDoTasksAsRSSCommand
...
- (void)buildQueryExpression {
  ...
  [[self query] appendFormat:@"    OR j.executant_id IN (%@) ", 
                             [self->teamIds componentsJoinedByString:@","]];
...
- (void)appendRSSItem:(NSDictionary *)_record {
  ...
  [title appendFormat:@" (%@)", tmp, [_record valueForKey:@"actor"]];
  ...
  guid = [NSString stringWithFormat:@"OGo-Task-%@-todo", tmp];
...
- (void)_executeInContext:(id)_context {
  ...
  self->teamIds = [[NSMutableArray alloc] initWithCapacity:[teams count]];
  enumerator = [teams objectEnumerator];
  while ((tmp = [enumerator nextObject]) != nil) {
    [self->teamIds addObject:[tmp valueForKey:@"companyId"]];
  }
...
@end /* LSGetToDoActionsAsRSSCommand */
