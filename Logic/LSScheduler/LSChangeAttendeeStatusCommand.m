...
/*
  appointment::change-attendee-status
...
@interface LSChangeAttendeeStatusCommand : LSDBObjectBaseCommand
{
  NSArray  *apts;
  NSString *logText;
  NSString *comment;
...
@implementation LSChangeAttendeeStatusCommand
...
- (void)_executeInContext:(LSCommandContext *)_cmdctx {
  ...
  if (![newValues isNotEmpty]) {
    [self debugWithFormat:@"nothing to update?"];
    return;
  }
  ...
  while ((pkey = [e nextObject]) != nil) {
    ...
    if (![assignment isNotNull]) {
      [self errorWithFormat:
	      @"login account is not an attendee of appointment with id: %@",
	      pkey];
      continue;
    }
...
@end /* LSChangeAttendeeStatusCommand */
