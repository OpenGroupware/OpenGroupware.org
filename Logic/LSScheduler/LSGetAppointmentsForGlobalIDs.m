...
- (id)_fetchAttributesInContext:(id)_context gids:(NSArray *)_gids {
  ...
  *(&batchSize) = gidCount > 200 ? 200 : gidCount;
  ...
  for (*(&i) = 0; i < gidCount; i += batchSize) {
    ...
    for (j = i; (j < (i+batchSize)) && (j < gidCount); j++) {
      ...
      if ([perm rangeOfString:@"v"].length > 0) {
        ...
        if ([k length] > 0) {
          ...
          [self logWithFormat:@"got invalid GID for perm 'v': %@", gid];
      }
        ...
        if ([k length] > 0) {
          if (listIn == nil) {
            listIn = [[NSMutableString alloc] initWithCapacity:batchSize * 4];
            [listIn appendString:@"%@ IN ("];
          } 
          else
            [listIn appendString:@","];
          [listIn appendString:k];
        }
        else
          [self logWithFormat:@"got invalid GID for perm 'l': %@", gid];
      }
    }
    if (in != nil) {
      [in appendString:@")"];
    ...
    while ((row = [allFetched nextObject])) {
      EOGlobalID *gid;
      ...
      if (getPerms) {
        ...
        [(NSMutableDictionary *)row 
          setObject:permissions forKey:@"permissions"];
      }
  ...
  if (([participantKeys count] > 0) && ([results count] > 0)) {
    ...
    pgids = LSRunCommandV(_context,
                          @"appointment", @"get-participants",
                          @"appointments",   resultGids,
    ...
    while ((participants = [e nextObject])) {
      unsigned i, count;
      ...
      for (i = 0, count = [participants count]; i < count; i++) {
        ...
        if ([[pgid entityName] isEqualToString:@"Person"])
          [personGids addObject:pgid];
        else
          [teamGids addObject:pgid];
      }
    }
    /* fetch companies */
    ...
    if ([teamGids count] > 0) {
      ...
      objs[count2++] = @"dbStatus";
      
      teamAttrs = [NSArrayClass arrayWithObjects:objs count:count2];
      free(objs);
    ...
    if ([personGids count] > 0) {
      ...
      persons = LSRunCommandV(_context,
    ...
    for (i = 0, count = [results count]; i < count; i++) {
      ...
      if ((psCount = [pgida count]) > 0) {
        ...
        a = [[NSArrayClass alloc] initWithObjects:ps count:j];
        free(ps); ps = NULL;
