...
%define ogoall_sope_source              sope-4.4rc.1-rock-r638.tar.gz
%define ogoall_sope_major               4
%define ogoall_sope_minor               4
%define ogoall_sope_makeflags           debug=yes
%define ogoall_ogo_source               opengroupware.org-5.4.0-ultra-r829.tar.gz
...
SHAREDIR_OGO="${RPM_BUILD_ROOT}%{prefix}/share/opengroupware.org-5.4"
SHAREDIR_ZIDE="${RPM_BUILD_ROOT}%{prefix}/share/zidestore-1.5"
...
touch ${INITSCRIPT_DST}/ogo-nhsd-1.1
touch ${INITSCRIPT_DST}/ogo-webui-1.1
touch ${INITSCRIPT_DST}/ogo-xmlrpcd-1.1
touch ${INITSCRIPT_DST}/ogo-zidestore-1.5
...
" >${RPM_BUILD_ROOT}%{_sysconfdir}/sysconfig/ogo-webui-1.1

echo "PGCLIENTENCODING=\"LATIN1\"           # client encoding to use
" >${RPM_BUILD_ROOT}%{_sysconfdir}/sysconfig/ogo-nhsd-1.1

echo "PGCLIENTENCODING=\"LATIN1\"           # client encoding to use
" >${RPM_BUILD_ROOT}%{_sysconfdir}/sysconfig/ogo-xmlrpcd-1.1

echo "PGCLIENTENCODING=\"LATIN1\"           # client encoding to use
" >${RPM_BUILD_ROOT}%{_sysconfdir}/sysconfig/ogo-zidestore-1.5
...
if [ $1 = 1 ]; then
  ...
  if [ -f "/etc/SuSE-release" ]; then
    ...
    sed "s^NHSD_INIT_VERSION^${NHSD_INIT_VERSION}^g; \
         s^NHSD_INIT_PREFIX^${NHSD_INIT_PREFIX}^g" \
         "%{prefix}/share/opengroupware.org-5.4/initscript_templates/redhat_nhsd" \
         >%{_sysconfdir}/init.d/"${NHSD_INIT_VERSION}"
    chown root:root %{_sysconfdir}/init.d/"${NHSD_INIT_VERSION}"
    chmod 755 %{_sysconfdir}/init.d/"${NHSD_INIT_VERSION}"
  ...
  ##
  OGO_INIT_VERSION="ogo-webui-1.1"
  OGO_INIT_PREFIX="%{prefix}"
  if [ -f "/etc/SuSE-release" ]; then
    sed "s^OGO_INIT_VERSION^${OGO_INIT_VERSION}^g; \
         s^OGO_INIT_PREFIX^${OGO_INIT_PREFIX}^g" \
         "%{prefix}/share/opengroupware.org-5.4/initscript_templates/suse_opengroupware" \
    ...
    sed "s^OGO_INIT_VERSION^${OGO_INIT_VERSION}^g; \
         s^OGO_INIT_PREFIX^${OGO_INIT_PREFIX}^g" \
         "%{prefix}/share/opengroupware.org-5.4/initscript_templates/redhat_opengroupware" \
         >%{_sysconfdir}/init.d/"${OGO_INIT_VERSION}"
    chown root:root %{_sysconfdir}/init.d/"${OGO_INIT_VERSION}"
    chmod 755 %{_sysconfdir}/init.d/"${OGO_INIT_VERSION}"
  ...
  if [ -f "/etc/SuSE-release" ]; then
    ...
    sed "s^XMLRPCD_INIT_VERSION^${XMLRPCD_INIT_VERSION}^g; \
         s^XMLRPCD_INIT_PREFIX^${XMLRPCD_INIT_PREFIX}^g" \
         "%{prefix}/share/opengroupware.org-5.4/initscript_templates/redhat_xmlrpcd" \
         >%{_sysconfdir}/init.d/"${XMLRPCD_INIT_VERSION}"
    chown root:root %{_sysconfdir}/init.d/"${XMLRPCD_INIT_VERSION}"
    chmod 755 %{_sysconfdir}/init.d/"${XMLRPCD_INIT_VERSION}"
  ...
  ##
  ZIDESTORE_INIT_VERSION="ogo-zidestore-1.5"
  ...
  if [ -f "/etc/SuSE-release" ]; then
    sed "s^ZIDESTORE_INIT_VERSION^${ZIDESTORE_INIT_VERSION}^g; \
         ...
         "%{prefix}/share/zidestore-1.5/initscript_templates/suse_zidestore" \
    ...
    sed "s^ZIDESTORE_INIT_VERSION^${ZIDESTORE_INIT_VERSION}^g; \
         s^ZIDESTORE_INIT_PREFIX^${ZIDESTORE_INIT_PREFIX}^g" \
         "%{prefix}/share/zidestore-1.5/initscript_templates/redhat_zidestore" \
         >%{_sysconfdir}/init.d/"${ZIDESTORE_INIT_VERSION}"
    chown root:root %{_sysconfdir}/init.d/"${ZIDESTORE_INIT_VERSION}"
    chmod 755 %{_sysconfdir}/init.d/"${ZIDESTORE_INIT_VERSION}"
  ...
  sed "s^OGO_SYSCONF^${OGO_SYSCONF}^g; \
       ...
       "%{prefix}/share/opengroupware.org-5.4/aptnotify_template/ogo-aptnotify.sh" \
  ...
  Defaults write ogo-nhsd-1.1 NGBundlePath '%{prefix}/lib/opengroupware.org-5.4/conduits'
  "
  ##
  chmod 755 ${OGO_HOME}
  ##
  if [ -f "%{prefix}/share/opengroupware.org-5.4/dbsetup/database_setup_psql.sh" ]; then
    %{prefix}/share/opengroupware.org-5.4/dbsetup/database_setup_psql.sh initial
  fi
fi
...
# ogo
%attr(0644,root,root) %config %{_sysconfdir}/sysconfig/ogo-webui-1.1
%ghost %attr(0755,root,root) %config %{_sysconfdir}/init.d/ogo-webui-1.1
%{prefix}/bin/load-LSModel
%{prefix}/bin/ogo-ppls-1.1
...
%{prefix}/share/opengroupware.org-5.4/aptnotify_template/ogo-aptnotify.sh
%{prefix}/lib/%{ogo_libogopalmui}.so.5.3*
%{prefix}/lib/%{ogo_libogopalm}.so.5.3*
%{prefix}/lib/libLSAddress*.so.5.3*
%{prefix}/lib/libLSFoundation*.so.5.3*
%{prefix}/lib/libLSSearch*.so.5.3*
%{prefix}/lib/libOGoAccounts*.so.5.3*
%{prefix}/lib/libOGoBase*.so.5.3*
%{prefix}/lib/libOGoContacts*.so.5.3*
%{prefix}/lib/libOGoDatabaseProject*.so.5.3*
%{prefix}/lib/libOGoDocuments*.so.5.3*
%{prefix}/lib/libOGoFileSystemProject*.so.5.3*
%{prefix}/lib/libOGoFoundation*.so.5.3*
%{prefix}/lib/libOGoJobs*.so.5.3*
%{prefix}/lib/libOGoNHS*.so.5.3*
%{prefix}/lib/libOGoProject*.so.5.3*
%{prefix}/lib/libOGoRawDatabase*.so.5.3*
%{prefix}/lib/libOGoScheduler*.so.5.3*
%{prefix}/lib/libOGoSchedulerTools*.so.5.3*
%{prefix}/lib/libOGoWebMail*.so.5.3*
%{prefix}/lib/libPPSync*.so.5.3*
%{prefix}/lib/libZSAppointments*.so.1.5*
%{prefix}/lib/libZSBackend*.so.1.5*
%{prefix}/lib/libZSContacts*.so.1.5*
%{prefix}/lib/libZSFrontend*.so.1.5*
%{prefix}/lib/libZSProjects*.so.1.5*
%{prefix}/lib/libZSTasks*.so.1.5*
%{prefix}/lib/opengroupware.org-5.4/commands/LSAccount.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSAddress.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSBase.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSDocuments.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSEnterprise.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSMail.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSNews.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSPerson.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSProject.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSScheduler.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSSearch.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSTasks.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/LSTeam.cmd
%{prefix}/lib/opengroupware.org-5.4/commands/OGo.model
%{prefix}/lib/opengroupware.org-5.4/conduits/OpenGroupwareNHS.conduit/OpenGroupwareNHS
%{prefix}/lib/opengroupware.org-5.4/conduits/OpenGroupwareNHS.conduit/Resources/Info-gnustep.plist
%{prefix}/lib/opengroupware.org-5.4/conduits/OpenGroupwareNHS.conduit/bundle-info.plist
%{prefix}/lib/opengroupware.org-5.4/conduits/OpenGroupwareNHS.conduit/stamp.make
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoAccounts.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoBase.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoContacts.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoDatabaseProject.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoFileSystemProject.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoJobs.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoPalmDS.ds/OGoPalmDS
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoPalmDS.ds/Resources/Info-gnustep.plist
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoPalmDS.ds/bundle-info.plist
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoPalmDS.ds/stamp.make
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoProject.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoRawDatabase.ds
%{prefix}/lib/opengroupware.org-5.4/datasources/OGoScheduler.ds
%{prefix}/lib/opengroupware.org-5.4/webui/AddressUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/AdminUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/BaseUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/EnterprisesUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/JobUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/LDAPAccounts.lso
%{prefix}/lib/opengroupware.org-5.4/webui/LSWMail.lso
%{prefix}/lib/opengroupware.org-5.4/webui/LSWProject.lso
%{prefix}/lib/opengroupware.org-5.4/webui/LSWScheduler.lso
%{prefix}/lib/opengroupware.org-5.4/webui/NewsUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoDocInlineViewers.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoMailEditor.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoMailFilter.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoMailInfo.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoMailViewers.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoNote.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoPalm.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoProject.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoProjectInfo.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoProjectZip.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoRecipientLists.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoResourceScheduler.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoScheduler.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoSchedulerDock.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoSchedulerViews.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoUIElements.lso
%{prefix}/lib/opengroupware.org-5.4/webui/OGoWebMail.lso
%{prefix}/lib/opengroupware.org-5.4/webui/PersonsUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/PreferencesUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/PropertiesUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/RelatedLinksUI.lso
%{prefix}/lib/opengroupware.org-5.4/webui/SoOGo.lso
%{prefix}/lib/zidestore-1.5
%{prefix}/sbin/ogo-nhsd-1.1
%attr(0644,root,root) %config %{_sysconfdir}/sysconfig/ogo-nhsd-1.1
%ghost %attr(0755,root,root) %config %{_sysconfdir}/init.d/ogo-nhsd-1.1
%{prefix}/sbin/ogo-webui-1.1
%{prefix}/sbin/ogo-xmlrpcd-1.1
%attr(0644,root,root) %config %{_sysconfdir}/sysconfig/ogo-xmlrpcd-1.1
%ghost %attr(0755,root,root) %config %{_sysconfdir}/init.d/ogo-xmlrpcd-1.1
%{prefix}/sbin/ogo-zidestore-1.5
%attr(0644,root,root) %config %{_sysconfdir}/sysconfig/ogo-zidestore-1.5
%ghost %attr(0755,root,root) %config %{_sysconfdir}/init.d/ogo-zidestore-1.5
%{prefix}/share/opengroupware.org-5.4/initscript_templates/*xmlrpcd
%{prefix}/share/zidestore-1.5
%{prefix}/share/opengroupware.org-5.4/INSTALLED.USING.OGOFULLPACKAGE
%{prefix}/share/opengroupware.org-5.4/Holidays.plist
%{prefix}/share/opengroupware.org-5.4/initscript_templates/*nhsd
%{prefix}/share/opengroupware.org-5.4/initscript_templates/*opengroupware
%{prefix}/share/opengroupware.org-5.4/templates/AddressUI
%{prefix}/share/opengroupware.org-5.4/templates/AdminUI
%{prefix}/share/opengroupware.org-5.4/templates/BaseUI
%{prefix}/share/opengroupware.org-5.4/templates/EnterprisesUI
%{prefix}/share/opengroupware.org-5.4/templates/JobUI
%{prefix}/share/opengroupware.org-5.4/templates/LDAPAccounts
%{prefix}/share/opengroupware.org-5.4/templates/LSWMail
%{prefix}/share/opengroupware.org-5.4/templates/LSWProject
%{prefix}/share/opengroupware.org-5.4/templates/LSWScheduler
%{prefix}/share/opengroupware.org-5.4/templates/NewsUI
%{prefix}/share/opengroupware.org-5.4/templates/OGoDocInlineViewers
%{prefix}/share/opengroupware.org-5.4/templates/OGoMailEditor
%{prefix}/share/opengroupware.org-5.4/templates/OGoMailFilter
%{prefix}/share/opengroupware.org-5.4/templates/OGoMailInfo
%{prefix}/share/opengroupware.org-5.4/templates/OGoMailViewers
%{prefix}/share/opengroupware.org-5.4/templates/OGoNote
%{prefix}/share/opengroupware.org-5.4/templates/OGoProject
%{prefix}/share/opengroupware.org-5.4/templates/OGoProjectInfo
%{prefix}/share/opengroupware.org-5.4/templates/OGoProjectZip
%{prefix}/share/opengroupware.org-5.4/templates/OGoRecipientLists
%{prefix}/share/opengroupware.org-5.4/templates/OGoResourceScheduler
%{prefix}/share/opengroupware.org-5.4/templates/OGoScheduler
%{prefix}/share/opengroupware.org-5.4/templates/OGoSchedulerDock
%{prefix}/share/opengroupware.org-5.4/templates/OGoSchedulerViews
%{prefix}/share/opengroupware.org-5.4/templates/OGoUIElements
%{prefix}/share/opengroupware.org-5.4/templates/OGoWebMail
%{prefix}/share/opengroupware.org-5.4/templates/PersonsUI
%{prefix}/share/opengroupware.org-5.4/templates/PreferencesUI
%{prefix}/share/opengroupware.org-5.4/templates/PropertiesUI
%{prefix}/share/opengroupware.org-5.4/templates/RelatedLinksUI
%{prefix}/share/opengroupware.org-5.4/templates/Themes/OOo
%{prefix}/share/opengroupware.org-5.4/templates/Themes/blue
%{prefix}/share/opengroupware.org-5.4/templates/Themes/kde
%{prefix}/share/opengroupware.org-5.4/templates/Themes/orange
%{prefix}/share/opengroupware.org-5.4/templates/ogo-webui-1.1
%{prefix}/share/opengroupware.org-5.4/www/Danish.lproj
%{prefix}/share/opengroupware.org-5.4/www/English.lproj
%{prefix}/share/opengroupware.org-5.4/www/English_OOo.lproj
%{prefix}/share/opengroupware.org-5.4/www/English_blue.lproj
%{prefix}/share/opengroupware.org-5.4/www/English_kde.lproj
%{prefix}/share/opengroupware.org-5.4/www/English_orange.lproj
%{prefix}/share/opengroupware.org-5.4/www/German.lproj
%{prefix}/share/opengroupware.org-5.4/www/German_OOo.lproj
%{prefix}/share/opengroupware.org-5.4/www/German_blue.lproj
%{prefix}/share/opengroupware.org-5.4/www/German_orange.lproj
%{prefix}/share/opengroupware.org-5.4/www/Italian.lproj
%{prefix}/share/opengroupware.org-5.4/www/Polish.lproj
%{prefix}/share/opengroupware.org-5.4/www/Spanish.lproj
%{prefix}/share/opengroupware.org-5.4/www/WOStats.xsl
%{prefix}/share/opengroupware.org-5.4/www/menu.js

# translations
%{prefix}/share/opengroupware.org-5.4/translations/Basque.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Danish.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Dutch.lproj
%{prefix}/share/opengroupware.org-5.4/translations/English.lproj
%{prefix}/share/opengroupware.org-5.4/translations/French.lproj
%{prefix}/share/opengroupware.org-5.4/translations/German.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Hungarian.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Italian.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Japanese.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Norwegian.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Polish.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Portuguese.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Slovak.lproj
%{prefix}/share/opengroupware.org-5.4/translations/Spanish.lproj
%{prefix}/share/opengroupware.org-5.4/translations/ptBR.lproj
...
# ogo-database-setup
%{prefix}/share/opengroupware.org-5.4/dbsetup
