...
sub build_rpm {
  ...
  if(($package eq "opengroupware") and ($build_type eq "trunk")) {
    ...
    foreach $line (@t_sope) {
      ...
      $rc = system("wget -q --proxy=off -O $ENV{HOME}/install_tmp/$sope_rpm http://$dl_host/releases/unstable/$use_sope/$host_i_runon/$sope_rpm") if($use_sope !~ m/^trunk/);
      $rc = system("wget -q --proxy=off -O $ENV{HOME}/install_tmp/$sope_rpm http://$dl_host/nightly/packages/$host_i_runon/$use_sope/$sope_rpm") if($use_sope =~ m/^trunk/);
      print " ...success!\n" if($rc == 0);
...
sub prepare_build_env {
  ...
  my $dir;
  my @dirs = qw( logs macros rpm rpm/BUILD rpm/RPMS rpm/RPMS/athlon rpm/RPMS/i386 rpm/RPMS/i486 rpm/RPMS/i586 rpm/RPMS/i686 rpm/RPMS/noarch rpm/SOURCES rpm/SPECS rpm/SPECS/initscript_templates rpm/SPECS/db_setup_template rpm/SPECS/aptnotify_template rpm/SRPMS rpm/tmp spec_tmp install_tmp );
  if (!$ENV{'HOME'}) {
