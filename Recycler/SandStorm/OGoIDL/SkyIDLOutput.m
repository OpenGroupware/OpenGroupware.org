...
static unsigned SkyIDLOutputNameCounter = 0;

@implementation SkyIDLOutput
...
@implementation SkyIDLOutput(SkyIDLSaxBuilder)
...
- (id)initWithAttributes:(id<SaxAttributes>)_attrs
  ...
  if ((self = [super initWithAttributes:_attrs
    ...
    if (self->name == nil) {
      self->name =
        [[NSString stringWithFormat:@"_part%d", SkyIDLOutputNameCounter++] copy];
    }
  }
  return self;
}
...
@end /* SkyIDLOutput(SkyIDLSaxBuilder) */
